...
export const CartProvider = ({ children }) => {
  ...
  // Load from local storage on mount (post-hydration)
  useEffect(() => {
    if (typeof localStorage !== "undefined") {
      try {
        ...
        if (savedState) {
          ...
          dispatch({ type: SET_CART_ITEMS, payload: { items: parsed.cartItems || [], cartId: parsed.cartId } });
        }
      } catch (e) {}
    }
  }, []);
  ...
  useEffect(() => {
    if (typeof localStorage !== "undefined") {
      try {
        ...
        if (savedState) {
          const parsed = JSON.parse(savedState);
          dispatchCompare({ type: 'SET_COMPARE', payload: parsed.compareProducts || [] });
        }
      } catch (e) {}
    }
  }, []);
