...
export const CategoryPage = (props) => {
  ...
  const [selectedCategory, setSelectedCategory] = useState(props.selectedCategory || null);
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(props.totalPages || 1);
  const [searchText, setSearchText] = useState("");
  const [filteredSuggestions, setFilteredSuggestions] = useState(props.products || []);
  ...
  useEffect(() => {
    if (props && props.categories && props.dynamicCategoryData) {
      setCategories(props.categories);
      setDynamicCategoryData(props.dynamicCategoryData);
      setSelectedCategory(props.selectedCategory || null);
  ...
  useEffect(() => {
    if (props && props.products && currentPage === 1 && selectedCategory && props.selectedCategory && (selectedCategory.category_name || selectedCategory.title) === (props.selectedCategory.category_name || props.selectedCategory.title)) {
      setFilteredSuggestions(props.products);
      setTotalPages(props.totalPages || 1);
      setTotalResults(props.totalResults || 0);
      return;
    }
