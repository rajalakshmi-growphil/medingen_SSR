...
export const SaltPage = (props) => {
  const [salt, setSalt] = useState(props.salt || null);
  const [alternateProducts, setAlternateProducts] = useState(props.alternateProducts || []);
  const [tabs, setTabs] = useState(props.tabs || {});
  const [activeTab, setActiveTab] = useState(props.activeTab || "");
  const [openFaq, setOpenFaq] = useState(null);          // which FAQ item is open
  const [loading, setLoading] = useState(props.salt ? false : true);
  const [productsLoading, setProductsLoading] = useState(props.alternateProducts ? false : true);
    ...
    try {
      ...
      if (response) {
        ...
        const desc = await axios.get(
          `/cloudfront-cdn/product_description/${response.description_url}`
        );
  ...
  useEffect(() => {
    if (props && props.salt && props.alternateProducts) {
      setSalt(props.salt);
      setAlternateProducts(props.alternateProducts);
      setLoading(false);
      setProductsLoading(false);
      if (props.descriptionHtml) {
        const sanitized = DOMPurify.sanitize(props.descriptionHtml);
        const parsedTabs = parseDescriptionToTabs(sanitized);
        setTabs(parsedTabs);
        setActiveTab(Object.keys(parsedTabs)[0] || "");
      }
                      ...
                      >
                        <div className="sp-product-img">
                          <img
                            ...
                            src={`/cloudfront-cdn/products/${p.imageUrl}`}
                            loading="lazy"
