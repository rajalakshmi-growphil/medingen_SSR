...
export const Dashboard = (props) => {
  ...
  const [loadingBanner, setLoadingBanner] = useState(props.slides ? false : true);
  const [allCategories, setAllCategories] = useState(props.allCategories || []);
  const [fullCategoryList, setFullCategoryList] = useState(props.fullCategoryList || []);
  const [activeMainCategory, setActiveMainCategory] = useState(null);
  const [activeCategory, setActiveCategory] = useState(props.allCategories?.[0]?.id || null);
  ...
  const [selectedCategoryGroup, setSelectedCategoryGroup] = useState(props.dynamicCategoryData?.[0]?.title || null);
  const [dynamicCategoryData, setDynamicCategoryData] = useState(props.dynamicCategoryData || []);
  const [mainCategoriesLoading, setMainCategoriesLoading] = useState(props.dynamicCategoryData ? false : true);
  ...
  useEffect(() => {
    if (props && props.categories && props.fullCategoryList && props.slides && props.dynamicCategoryData) {
      // Bypassed fetch due to server-side props
      return;
    }
  ...
  useEffect(() => {
    if (props && props.products && activeCategory && props.allCategories?.[0]?.id === activeCategory && products.length > 0) {
      // Bypassed fetch due to server-side props
      return;
    }
  ...
  const getSubcategoryImage = (subCatName) => {
    ...
    if (match) {
      if (match.category_image_url && match.category_image_url.trim() !== "")
        return `/cloudfront-cdn/categories/${match.category_image_url.trim()}`;
      if (match.category_outline_url && match.category_outline_url.trim() !== "")
        return `/cloudfront-cdn/categories/${match.category_outline_url.trim()}`;
    }
                    ...
                    <SwiperSlide key={`banner-${index}`}>
                      <div className="overlap-group-wrapper">
                        {slide.banner ? (
                          <img
                            className="banner"
                            src={`/cloudfront-cdn/banner/${slide.banner}`}
                            alt="Banner"
