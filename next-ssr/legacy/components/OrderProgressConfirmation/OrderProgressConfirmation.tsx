...
export const OrderProgress = () => {
  ...
  const [cart_id_external, setCartIdExternal] = useState<any>(null);
  
  useEffect(() => {
    if (location && location.state && location.state.cart_id) {
      setCartIdExternal(location.state.cart_id);
    }
  }, [location]);

  const [deliveryType, setDeliveryType] = useState("normal");
  const [activeCartId, setActiveCartId] = useState<any>(null);
  const navigate = useNavigate();
