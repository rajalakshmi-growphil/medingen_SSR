...
import { getUser, getDefaultAddress } from "../../../lib/api";
...
const HeadActions: React.FC<HeadActionsProps> = ({ variant }) => {
  ...
  useEffect(() => {
    ...
    setHideRoutes(includeRoutes.includes(pathname));
  }, [pathname]);
    ...
    <div className={`header-actions header-search-layout ${isCheckout ? "checkout-mode" : ""}`}>
      <>
        ...
        {!isCheckout && userData?.customer_id && (
          <div onClick={() => router.push("/notification")} className="action-icon-wrapper clickable">
